...
WIKIPEDIA_PAGE_RAW_CONTENT_URL = "https://en.wikipedia.org/w/index.php?title={title}&action=raw"



class RealWikiClient(BaseWikiClient):
    ...
    async def get_full_page_content(self, session: aiohttp.ClientSession, title: str) -> str:
        async with session.get(WIKIPEDIA_PAGE_RAW_CONTENT_URL.format(title=title)) as response:
